...
import { SearchParams, SearchResults as SearchResultsType } from '@/types/search'
...
export default function SearchPage() {
  ...
  const handleSearch = async (params: SearchParams) => {
    ...
    try {
      ...
      const secondaryQuery = params.secondaryKeywords.length > 0 
      ...
      const [redditResults, youtubeResults] = await Promise.allSettled([
        searchReddit(query, params.timeFrame, params.resultCount),
        searchYouTube(query, params.resultCount)
      ])

      const redditData = redditResults.status === 'fulfilled' ? redditResults.value : []
      const youtubeData = youtubeResults.status === 'fulfilled' ? youtubeResults.value : []
  ...
  return (
    <div className="container py-8 max-w-4xl">
      <div className="space-y-4 mb-8">
        <h1 className="font-lexend text-3xl font-bold tracking-tight">Search</h1>
        <p className="text-muted-foreground">
          Search across multiple platforms using keywords to find relevant discussions and opportunities.
        </p>
      </div>
